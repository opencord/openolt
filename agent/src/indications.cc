        ...
        case BCMOLT_OBJ_ID_ONU:
            switch (msg->subgroup) {
                case BCMOLT_ONU_AUTO_SUBGROUP_ONU_DISABLE_COMPLETED:
                {
                    ...
                    bcmolt_serial_number *in_serial_number = &(dgi_data->data.serial_number);


                    OPENOLT_LOG(INFO, openolt_log_id, "onu disable indication, pon_ni %d, onu_id %d, port_no %d, status %s, serial_number %s\n",
                        key->pon_ni, key->onu_id, port_no, bcmolt_result_to_string(dgi_data->data.status).c_str(),serial_number_to_str(in_serial_number).c_str());
